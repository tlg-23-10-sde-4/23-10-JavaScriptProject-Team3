...
async function getBodyComposition(accessToken) {
  console.log('accessToken', accessToken);
  try {
    const response = await axios.get(`${FITBIT_API_BASE_URL}/1/user/-/body/log/weight/date/${todayString}.json`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    return response.data;
  } catch (error) {
...
module.exports = { getUserProfile, getBodyComposition, getNutritionData, getActivitiesData, getSleepData };
